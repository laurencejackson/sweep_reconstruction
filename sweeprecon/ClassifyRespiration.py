...
class ClassifyRespiration(object):
    ...
    def bounds_peaks(self):
        ...
        conn_pos = np.interp(xx, peaks_pos, self._resp_trace[peaks_pos] + (0.9 * self._resp_trace[peaks_pos]))
        conn_neg = np.interp(xx, peaks_neg, self._resp_trace[peaks_neg] + (0.9 * self._resp_trace[peaks_neg]))
